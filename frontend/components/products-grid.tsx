...
export function ProductsGrid() {
  const { items: products, isLoading, isLoadingMore, hasMore, error, sentinelRef, reset } = useInfiniteScroll({
  ...
  return (
    <>
      ...
      <div ref={sentinelRef} className="mt-8 py-4 text-center">
        {isLoadingMore && (
          <div className="text-sm text-muted-foreground">商品を読み込み中...</div>
        )}
        {!hasMore && products.length > 0 && (
          <div className="text-sm text-muted-foreground">すべての商品を表示しました。</div>
        )}
      </div>
    </>
  );
